...
app.get('/speech-to-text', (req, res) => {
  ...
  const tcount = response.length //transcript 개수//
  /*const startseconds = response[0].alternatives[0].words[0].startTime.hasOwnProperty("seconds") ? Number(response[0].alternatives[0].words[2].startTime.seconds) : 0;
  const endseconds = response[0].alternatives[0].words[0].endTime.hasOwnProperty("seconds") ? 
  Number(response[0].alternatives[0].words[1].endTime.seconds) : 0;
  var transcript=response[1].alternatives[0].transcript
  var startTime=(response[0].alternatives[0].words[2].startTime.nanos)/1000000000+startseconds
  var endTime=(response[0].alternatives[0].words[1].endTime.nanos)/1000000000+endseconds*/
  //collecting all words : 모든 words배열 수집
  var script=""; //전체 음성인식된 script 출력
  for(var i=0;i<tcount;i++)
  {
    var wcount=response[i].alternatives[0].words.length //한 transcript의 단어 개수
    var transcript=response[i].alternatives[0].transcript
    var script=script.concat(transcript)
    for(var j=0;j<wcount;j++)
    {
      var startseconds = response[i].alternatives[0].words[j].startTime.hasOwnProperty("seconds") ? Number(response[i].alternatives[0].words[j].startTime.seconds) : 0;
      var startnanos = response[i].alternatives[0].words[j].startTime.hasOwnProperty("nanos") ? Number(response[i].alternatives[0].words[j].startTime.nanos) : 0;
      var endseconds = response[i].alternatives[0].words[j].endTime.hasOwnProperty("seconds") ? 
      Number(response[i].alternatives[0].words[j].endTime.seconds) : 0;
      var endnanos = response[i].alternatives[0].words[j].endTime.hasOwnProperty("nanos") ? 
      Number(response[i].alternatives[0].words[j].endTime.nanos) : 0;
      var startTime=(response[i].alternatives[0].words[j].startTime.nanos)/1000000000+startseconds
      var endTime=(response[i].alternatives[0].words[j].endTime.nanos)/1000000000+endseconds
      var word=response[i].alternatives[0].words[j].word
      var lpw=(endTime-startTime)/(word.length)
      response[i].alternatives[0].words[j].lpw=lpw //단어별 빠르기 계산하여 json에 추가
    }
  }

  const result = {
    asrResult: String(script)
    //asrResult: String(response[0].alternatives[0].confidence)
    //asrResult: String(response[0].alternatives[0].words[0].lpw)
    //asrResult: String((response[0].alternatives[0].words[0].endTime.nanos)/1000000000+endseconds)
    //asrResult: response[0].alternatives[0].words[0].word//
  };
  res.status(200).send(result.asrResult);
